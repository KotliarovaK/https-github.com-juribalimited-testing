...
Scenario: EvergreenJnr_AllLists_CheckThat500ErrorIsNotDisplayedForDynamicListAfterRemovingAssociationsList
	When User add following columns using URL to the "Applications" page:
		| ColumnName      |
		| Application Key |
	When User create dynamic list with "DAS11468List" name on "Applications" page
	Then "DAS11468List" list is displayed to user
	When User clicks 'Devices' on the left-hand menu
	Then 'All Devices' list should be displayed to the user
	When User clicks the Filters button
	Then Filters panel is displayed to the user
	When User add "Application (Saved List)" filter where type is "In list" with Selected Value and following Association:
		| SelectedList | Association        |
		| DAS11468List | Not used on device |
	Then "Any Application in list DAS11468List not used on device" is displayed in added filter info
	When User create dynamic list with "DAS11468List" name on "Devices" page
	Then "DAS11468List" list is displayed to user
	When User clicks 'Applications' on the left-hand menu
	Then 'All Applications' list should be displayed to the user
	Then User remove list with "DAS11468List" name on "Applications" page
	When User clicks 'Devices' on the left-hand menu
	Then 'All Devices' list should be displayed to the user
	When User navigates to the "DAS11468List" list
	Then "DAS11468List" list is displayed to user
